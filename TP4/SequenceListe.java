...
class SequenceListe<Type> implements Sequence<Type> {
	Maillon<Type> tete, queue;

	// Les méthodes implémentant l'interface
	// doivent être publiques
	@Override
	public void insereTete(Type element) {
		Maillon<Type> m = new Maillon<>(element, tete);
		if (queue == null)
			queue = m;
		tete = m;
	}
	...
	public void insereQueue(Type element) {
		Maillon<Type> m = new Maillon<>(element, null);
		if (queue == null) {
			tete = queue = m;
		} else {
			queue.suivant = m;
	...
	public Iterateur<Type> iterateur(){
		Iterateur<Type> it = new IterateurListe<>(this);
		return it; 
	}
}
